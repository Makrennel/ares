...
public class AutoSurround extends Module {
    ...
    private final Setting<Integer> holeDelay = register(new IntegerSetting("Hole Delay", 250, 0, 1000)).setVisibility(hole::getValue);
    ...
    public void onTick() {
        if (WorldUtils.isHole(MC.player.getBlockPos()) != HoleType.NONE && !Surround.INSTANCE.getEnabled() && hole.getValue()) {
            ...
            Surround.setSurroundDelay(holeDelay.getValue());
        }
    }
}
